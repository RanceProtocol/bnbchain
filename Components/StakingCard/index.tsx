...
const StakingCard: FC<IProps> = (props) => {
    ...
    return (
        <Fragment>
            ...
            <h2 className={styles.pool__tokens}>{`${stakeTokenSymbol} - ${rewardTokenSymbol}`}</h2>
            <p className={styles.pool__description}>{`Stake ${stakeTokenSymbol}, Earn ${rewardTokenSymbol}`}</p>
            ...
            <div className={styles.pool__details}>
                <div className = {styles.key__values}>
                    ...
                    <span className={styles.value}>{`$${Number(utils.formatUnits(totalEarned, rewardTokenDecimals)) * rewardTokenPrice}`}</span>
                </div>

                <div className = {styles.key__values}>
                    <span className={styles.key}>Total Staked</span>
                    <span className={styles.value}>{`$${Number(utils.formatUnits(totalStaked, stakeTokenDecimals)) * stakeTokenPrice}`}</span>
                </div>
            </div>

            {userEarned !== undefined && <div className={styles.user__details}>
                <div className = {styles.key__values}>
                    <span className={styles.key}>Earnings</span>
                    <span className={styles.value}>{`${Number(utils.formatUnits(userEarned!, rewardTokenDecimals))} ~ $${Number(utils.formatUnits(userEarned!, rewardTokenDecimals)) * rewardTokenPrice}`}</span>
                </div>

                <div className = {styles.key__values}>
                    <span className={styles.key}>Stake</span>
                    <span className={styles.value}>{`${Number(utils.formatUnits(userStaked!, stakeTokenDecimals))} ~ $${Number(utils.formatUnits(userStaked!, stakeTokenDecimals)) * stakeTokenPrice}`}</span>
                </div>
            </div>}
            {/* {!account ? 
...
{account ? (
            ...
            <a className={styles.contract__link} href={`https://cronos.com/address/${contractAddress}`} target = "_blank" rel="noreferrer">view contract</a>
        </div>
            {/* <StakingModal 
